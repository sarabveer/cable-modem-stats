...
def get_credential(config):
    ...
    data = {"username": config["modem_username"], "password": config["modem_password"], "locale": False}

    try:
        resp = requests.post(url, data=data, allow_redirects=False, timeout=config["request_timeout"])
...
def parse_html(html):
    ...
    for i in range(upstream_channels):
        ...
        # Modulation naming is a bit different for the xb8 than arris
        channel_type = (
            upstream_rows[5].find_all("td")[i].text.strip() + "-" + upstream_rows[6].find_all("td")[i].text.strip()
        )
