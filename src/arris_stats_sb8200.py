...
def get_credential(config):
    ...
    auth_prefix = "?login_" if config["modem_new_auth"] else "?"
    auth_url = url + auth_prefix + auth_hash
    ...
    try:
        if config["modem_new_auth"]:
            ...
            resp = requests.get(
                auth_url, auth=(username, password), verify=verify_ssl, timeout=config["request_timeout"]
            )
            cookie = None
...
def get_html(config, credential):
    ...
    logging.info("Retrieving stats from %s", init_url)

    try:
        resp = requests.get(url, cookies=cookies, verify=verify_ssl, timeout=config["request_timeout"])
        if resp.status_code != 200:
