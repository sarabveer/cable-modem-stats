...
def main():
    ...
    while True:
        ...
        if destination == "influxdb":
            ...
            error_exit(f"Destination {destination} not supported!  Aborting.", sleep=False)
...
def send_to_influx(stats, config):
    ...
    for stats_down in stats["downstream"]:
        data = {
            ...
            "tags": {"channel_id": int(stats_down["channel_id"]), "modulation": stats_down["modulation"]},
        }
            ...
            Point.from_dict(
                {
                    ...
                    "tags": {"channel_id": int(stats_up["channel_id"]), "channel_type": stats_up["channel_type"]},
                }
            )
        )
...
def str_to_bool(string, name):
    ...
    raise ValueError(f'Config parameter {name} should be boolean "true" or "false", but value is neither of those.')
